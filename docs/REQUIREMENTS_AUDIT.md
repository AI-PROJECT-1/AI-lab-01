...
| R-02 | Playable desktop or web GUI | 4.1 | `gui/app.py`, `gui/board_view.py`, `main.py` | GUI unit/E2E smoke | PASS | Clean `python main.py` launch produced a live `Griductive` window | Desktop display and Tkinter are required |
| R-03 | NxN board, coordinates, public identity/status, face-up clues and face-down cards | 4.1 | `gui/board_view.py`, `gui/character_card.py`, `gui/clue_panel.py` | GUI/card/clue/completion tests | PASS | 3x3 and 4x4 visual audit; all 16 low-height status badges visible | No external portraits are required |
...
| R-06 | Select a revealed clue and highlight referenced/counted cells | 4.1 | `gui/clue_panel.py`, `gui/controller.py`, `logic/region_resolver.py` | clue presentation and E2E tests | PASS | Canonical resolver drives explicit, row, column, and neighbor highlights | Puzzle data mainly exercises EXPLICIT; other regions are exhaustively tested |
| R-07 | Load, Restart, Hint, Auto Solve controls | 4.1 | `gui/app.py`, `gui/controls.py`, `game/game_engine.py` | lifecycle, Hint, completion tests | PASS | E2E covered Load/Restart/Hint/Solve Next/Auto Solve | Solve Next and Solver Details are additional UI affordances |
...
| R-10 | FACT, SAME, DIFFERENT, EXACTLY, AT_LEAST, AT_MOST | 3.2, 4.2 | `core/clue.py`, `logic/cnf_encoder.py`, `logic/semantic_evaluator.py` | exhaustive semantics/CNF tests | PASS | All six templates have independent evaluator and CNF coverage | Distributable chains emphasize FACT/SAME/DIFFERENT; counting types are test-backed |
| R-11 | ROW, COLUMN, 8-NEIGHBORS excluding self, and EXPLICIT regions | 2.1, 3.2 | `core/region.py`, `logic/region_resolver.py` | edge/corner/interior/explicit tests | PASS | One canonical resolver is reused by CNF, evaluator, and GUI | Shipped examples use EXPLICIT; all region families are synthetic-test-backed |
...
| R-20 | Several 3x3/4x4 experiments with counts, SAT work, runtime, reveal waves, failures retained | 4.5 | `experiments/run_experiments.py`, `experiments/results/final_regression.json` | `tests/test_final_audit.py` | PASS | Four puzzles, 0 failures; raw JSON retained with a reproducible command | Runtime is machine-specific; no optional 5x5 experiment |
...
| EXACTLY | structured `k` + canonical region | AT_LEAST + AT_MOST | `test_exactly_row_clue`, zero/all/explicit cases | PASS |
| AT_LEAST | structured `k` + canonical region | direct positive subset clauses | zero/nontrivial tests | PASS |
| AT_MOST | structured `k` + canonical region | direct negative subset clauses | column/neighbor tests | PASS |
| ROW | `RegionResolver.referenced_cells` | same resolver used by encoder | `test_row_column_and_explicit_regions`; EXACTLY-row test | PASS |
| COLUMN | same canonical resolver | same resolver used by encoder | row/column test; AT_MOST-column test | PASS |
| NEIGHBORS | excludes owner; clips at board boundary | same resolver used by encoder | `test_corner_edge_and_interior_neighbors`; AT_MOST-neighbor test | PASS |
...
Representative shipped data does not cover every core count/region combination;
that limitation is explicit in `KNOWN_LIMITATIONS.md`. Support is established by
executable model/evaluator/encoder paths plus exhaustive or boundary tests, not
by enum membership alone.
...
- The 4-puzzle suite contains two 3x3 and two 4x4 puzzles. All four are
